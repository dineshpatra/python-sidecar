...
class Env():
    ...
    def __init__(self):
        ...
        data = '{ "auth": {  "identity": { "methods": ["password" ], "password": {"user": { "name": "admin", "domain": {  "id": "default" }, "password": "demo" } } } }}'
        url = 'http://198.100.181.66:35357/v3/auth/tokens'
        response = requests.post(url, data = data, headers = headers)
        self.token = response.headers['X-Subject-Token']
        self.url = 'http://127.0.0.1:9090/user/'
    ...
    def create_data(self, args):
        ...
        #x.add_row([json.loads(response.text)])
        #print x
